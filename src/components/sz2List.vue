<template>
  <div class="sz2List">
    ...
    <div class="content" v-loading.body="loading">
      <div class="menu">
        <router-link to="/fx2" class='menu-title'><img src="../assets/back.png" alt="返回"> 返回
        </router-link>
        <span>{{ category }}</span>
        <router-link to="/fx2" class='menu-title'><img src="../assets/home.png" alt="首页"> 首页</router-link>
      </div>
      <div class="wrapper">
        <el-row :gutter="10">
...
export default {
  ...
  methods: {
    ...
    detail (el) {
      ...
      this.$router.push({path: '/sz2Detail/' + id + '/' + cid})
    }
  }
}
</script>
<style scoped>
  ...
  .sz2List .content .menu .menu-title {
    font-size: 13px;
    color: #fff;
    text-decoration: none;
  }
  .sz2List .content .menu .menu-title img{
    width: 13px;
    height: 13px;
    margin-bottom: -1px;
  }

  .sz2List .content .wrapper {
    font-family: 'kaiti';
    /*font-family: 'pinyin';*/
    flex: 1;
    overflow-y: auto;
  ...
  .sz2List .content .wrapper .w-col {
    padding: 5px 0 0 0;
  }

  .sz2List .content .wrapper .w-content {
    font-size: 40px;
    color: #000000;
    font-weight: 400;
  }
  .sz2List .content .wrapper .w-content .box-card{
    border-color: #038384;
