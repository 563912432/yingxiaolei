  ...
  <div class="shizi1">
    ...
    <div>
      <mu-dialog :open="dialog" @close="close">
        <div>完整版需购买产品后开通！</div>
        <!--<div class="sail"><a href="http://shop111181.top/W48vLury">点右上角，分享到微信朋友圈，凭朋友圈截图，立享3元店铺无门槛优惠券</a></div>-->
  ...
  methods: {
    ...
    click (el) {
      ...
      this.post(url, formdata, res => {
        this.loading = false
        if (res.status) {
          this.$router.push({path: '/sz1List/' + el})
        } else {
          this.dialog = true
        }
      })
...
<style scoped>
  ...
  .shizi1 .content ul li {
    padding: 10px;
  }

  .shizi1 .content .green {
    background-color: #bbd7d8;
    margin: 3px;
    border-radius: 3px;
  }

  .shizi1 .content .white {
    background-color: #ffffff;
    margin: 3px;
    border-radius: 3px;
  }
  .shizi1 .img-container {
    width: 50%;
    height: auto;
    text-align: center;
    align-self: center;
